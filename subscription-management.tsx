...
export default function SubscriptionManagement({
  ...
  const planDetails =
    currentPlan === "free"
      ...
      : SUBSCRIPTION_PLANS.find((plan) => plan.id === currentPlan) || {
          name: "Free",
          price: 0,
  ...
  const handleCancelSubscription = async () => {
    if (!subscriptionStatus?.id) return
    ...
    <div className="bg-card rounded-lg border p-6">
      ...
      <div className="space-y-4">
        ...
        <div className="pt-4 flex flex-col sm:flex-row gap-3">
          ...
          {currentPlan !== "free" && !subscriptionStatus?.cancelAtPeriodEnd && (
            <button
        ...
        {currentPlan !== "free" && subscriptionStatus?.cancelAtPeriodEnd && (
          <div className="mt-2 p-3 bg-yellow-50 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-200 rounded-md text-sm">
            ...
            <button onClick={() => setIsModalOpen(true)} className="text-primary hover:text-primary/80 text-xs mt-1">
